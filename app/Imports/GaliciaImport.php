...
class GaliciaImport implements ToModel, WithHeadingRow {
    /**
    * @param array $row
    *
    ...
    public function model(array $row) {
        try {
            if (isset($row["documento"]) && $row["documento"] != NULL) {
                ...
                if ($persona == NULL){
                    $persona = Personas::add($row["documento"], $row["nombres"]);
                }
                
                
                //Recorremos cada columna
                $database_array = array();
                $keys_that_we_dont_insert = ["nombres", "documento"];
                ...
                return new Galicia($database_array);
            }
        }
        catch (\Exception $e) {
